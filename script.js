...
function createNews(type, context) {
    ...
    // 将百分比转换为 40px 到 200px 之间的字体大小
    let fontSize = 40 + (fontSizePercent / 500) * (200 - 40);  // 百分比值转换为 40px 到 200px 之间

    let img = document.getElementById("main_img");
    img.innerHTML = `
        <img class="bg" src="./img/${type}.webp" />
        <p class="context" style="font-size: ${fontSize}px; ${type == 'good_news' ? 
