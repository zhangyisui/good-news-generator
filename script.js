...
function createNews(type, context) {
  ...
  img.innerHTML = `
        ...
        <p class="context" style="font-size: ${fontSize}px; ${
    type == "good_news"
...
function createNewsByImage(type, contextBase64) {
  ...
  img.innerHTML = `
        <img class="bg" src="./img/${type}.webp" />
        <img class="ft" src="${contextBase64}" ${
    type == "bad_news" ? 'style="filter: grayscale(1);"' : ""
  } />
    `;
}
...
window.addEventListener("load", () => {
  ...
  document.getElementById("good").onchange = document.getElementById(
    "bad"
  ).onchange = () => {
    if (picBase64 === null) {
      createNews(getNewsType(), document.getElementById("main_context").value);
  ...
  document.getElementById("font-size").addEventListener("input", () => {
    let fontSizePercent = document.getElementById("font-size").value;
    createNews(getNewsType(), document.getElementById("main_context").value);
  });
  ...
  document.getElementById("main_share").addEventListener("click", () => {
    domtoimage.toBlob(document.getElementById("main_img")).then((blob) => {
      navigator
        .share({
          files: [
            new File([blob], `${getNewsType()}.png`, { type: "image/png" }),
          ],
        })
        .then(() => {
          alert("分享成功！");
